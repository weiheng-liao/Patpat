    ...
    def init_subdir():
        ...
        for dir_ in dir_list:
            try:
                ...
                print(
                    f"\033[0;31mWarning! This directory already exists. 警告！该目录已经存在。[patpat_env/{dir_}]\033[0m"
                )

    if os.path.exists("patpat_env/"):
        print(
            "\033[0;31mWarning! This directory already exists. 警告！该目录已经存在。[patpat_env/]\033[0m"
        )
        init_subdir()
    else:
        os.mkdir("patpat_env/")
...
def list_local_proteome():
    ...
    return [
        re.search("(?<=uniprot-proteome_).*(?=.fasta)", p).group()
        for p in proteome
        if re.search("(?<=uniprot-proteome_).*(?=.fasta)", p)
    ]
...
def download_uniprot_opg(taxonomy_id):
    ...
    ftp.login()
    ftp.cwd(
        f"./pub/databases/uniprot/current_release/"
    ...
    with open(f"patpat_env/proteome/{file_name}.gz", mode="wb") as f_gz:
        ftp.retrbinary(f"RETR {file_name}.gz", f_gz.write)
    ftp.close()

    gz = gzip.GzipFile(f"patpat_env/proteome/{file_name}.gz", mode="rb")
    with open(
        f"patpat_env/proteome/uniprot-proteome_{up_id}_OGP_{download_time}.fasta",
        mode="wb",
    ) as fo:
        fo.write(gz.read())
    gz.close()
...
def download_uniprot_proteome(identifier: str):
    ...
    download_time = time.strftime("%Y-%m-%d", time.localtime())
    fasta_name = (
        "_".join(["patpat_env/proteome/uniprot-proteome", identifier, download_time])
        + ".fasta"
    )
...
def pagination_download_uniprot_proteome(
    identifier: str, reviewed: bool = True, size: int = 500
):
    """Under Development..."""
    if reviewed:
        reviewed = "true"
